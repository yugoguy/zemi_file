...
class Agent {
    ...
    public $decision; // Store the agent's decision

    public function __construct($num_states, $num_actions, $temperature, $lr, $endowment, $MPCR, $transparency) {
        $actions = $this->linspace(0, $endowment, $num_actions);
        $this->Q = array();
        for ($i = 0; $i < $num_states; $i++) {
            $q_row = array();
            foreach ($actions as $action) {
                $mean = $endowment - $action + ($MPCR) * $action;
        ...
        $this->decision = null; // Initialize the decision property
    }
    ...
    public function choose_action($use_most_likely_state = false) {
        ...
        $this->decision = $action; // Store the action index
...
class Environment {
    ...
    public $result;
    public $record;
    public $agent_contributions; // Store agents' contributions

    public function __construct($N, $E, $MR, $num_rounds, $temperature=10, $lr=0.5, $num_states=11, $num_actions=11, $transparency=1) {
        $this->N = $N;                  
        $this->E = $E;                
        $this->MR = $MR;            
        ...
        $this->agents = array();
        for ($i = 0; $i < $N; $i++) {
            $this->agents[] = new Agent($this->num_states, $this->num_actions, $this->initial_temperature, $this->lr, $this->E, $this->MR / $this->N, $this->transparency);
        }
        $this->action_values = $this->linspace(0, $this->E, $this->num_actions);
        $this->agent_contributions = array();
    }
    ...
    public function run_simulation() {
        ...
        for ($round_num = 0; $round_num < $this->num_rounds; $round_num++) {
            ...
            $payoff_per_agent = ($this->MR * $this->transparency * $total_contribution) / $this->N;
            $payoffs = array();
        ...
        // Store the contributions of each agent
        $this->agent_contributions = array();
        foreach ($this->agents as $agent) {
            $contribution = $this->action_values[$agent->decision];
            $this->agent_contributions[] = $contribution;
        }
    }
}
...
// Handle AJAX request
if (isset($_POST['ajax'])) {
    // Get the transparency value from POST data
    $transparency = floatval($_POST['transparency']);
    ...
    // Run the simulation
    $env = new Environment($N, $E, $MR, $num_rounds, $temperature, $lr, $num_states, $num_actions, $transparency);
    $env->run_simulation();

    // Prepare the response data
    $response = array(
        'average_final_contribution' => $env->result,
        'agent_contributions' => $env->agent_contributions,
        'N' => $N,
        'E' => $E
    );
